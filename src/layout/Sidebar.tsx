    ...
    <>
      ...
      <div
        className={`bg-white h-screen p-3 shadow-lg ${
          isMobile ? "fixed left-0 top-0 z-50 transition-transform" : "relative"
        } ${
          isMobile
            ? showSideBar
              ? "translate-x-0"
      ...
      >
        ...
        <hr className="h-px mt-[0.76rem] bg-transparent bg-gradient-to-r from-transparent via-black/40 to-transparent" />

        {/* Navigation Links */}
        <div className="flex flex-col gap-2 mt-4 max-h-[-webkit-fill-available] overflow-auto">
          {sidebars.map((item, idx) => {
            ...
            return (
              <NavLink
                ...
                onClick={() => {
                  if (isMobile) {
                    setShowSideBar(false); // Close sidebar when a link is clicked (only for mobile)
                  }
                }}
              >
